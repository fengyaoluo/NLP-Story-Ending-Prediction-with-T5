...
valid_logger = Table(
    Column("Loss1", justify = "center"),
    Column("Loss2", justify = "center"),
...
class LightningModel(pl.LightningModule):
    ...
    def training_epoch_end(self, training_step_outputs):
        ...
        training_logger.add_row(str(avg_traning_loss))
    ...
    #     # valid_logger.add_row(str(validation_step_outputs[0].item()),str(validation_step_outputs[1].item()))
    #     val_loss =np.round(torch.mean(torch.stack([x["loss"] for x in validation_step_outputs[0]])).item(),4,)
    #     console.log(f'[MODEL VALIDATION]')
    #     valid_logger.add_row(str(val_loss))
    #     console.log(valid_logger)
    #     path = f"{self.outputdir}/T5-epoch-{self.current_epoch}-val-loss-{str(val_loss)}"
    #     self.tokenizer.save_pretrained(path)
    #     self.model.save_pretrained(path)
